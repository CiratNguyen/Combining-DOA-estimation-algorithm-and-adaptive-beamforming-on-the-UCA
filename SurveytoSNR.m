...
SNR_0=5:1:50; % Tỷ số tín hiệu trên nhiễu
mean_PPAR = zeros(1,length(SNR_0));
DOA_RMSE = zeros(1,length(SNR_0));
...
for SNR_idx = 1:length(SNR_0)
    ...
    mean_PPAR(1,SNR_idx) = 10*log10(mean(PPAR));
    DOA_RMSE(1,SNR_idx) = mean(RMSE);
end