...
R_0=(0.05:0.01:0.5)*Ne*lambda_max/(2*pi); % Bán kính mảng
mean_PPAR = zeros(1,length(R_0));
DOA_RMSE = zeros(1,length(R_0));
for R_idx = 1:length(R_0)
    R = R_0(R_idx);
    for kk=1:1000
    ...
    mean_PPAR(1,R_idx) = 10*log10(mean(PPAR));
    DOA_RMSE(1,R_idx) = mean(RMSE);
end