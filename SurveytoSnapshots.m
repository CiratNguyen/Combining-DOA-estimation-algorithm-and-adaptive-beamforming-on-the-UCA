...
time_0 = 0.01:0.01:0.2;
mean_PPAR = zeros(1,length(time_0));
DOA_RMSE = zeros(1,length(time_0));
for time_idx = 1:length(time_0)
    time = time_0(time_idx);
    for kk=1:1000
        t0=Ts:Ts:time; % Thời gian lấy mẫu (s)
        s0_A=1*sin(2*pi*f1*t0).'; % Signal_A (mV)
        s0_B=1*sin(2*pi*f2*t0).'; % Signal_B (mV)
        s0_C=1*sin(2*pi*f3*t0).'; % Signal_C (mV)
        ...
        RMSE(kk) = sqrt((DOA_error_A^2 + DOA_error_B^2 + DOA_error_C^2)/D);
    end
    mean_PPAR(1,time_idx) = 10*log10(mean(PPAR));
    DOA_RMSE(1,time_idx) = mean(RMSE);
end